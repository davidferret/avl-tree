...
bool readStocksCSV(AVLTree<Stock>& stockTree, const std::string& filename = "stocks.csv");
...
int main() {
    ...
    std::cout << "\nHeight: " << intTree.height() << std::endl;

    // Use the new CSV file reader (ensure "stocks.csv" exists in your working directory)
    if (readStocksCSV(stockTree, "stocks.csv"))
...
bool readStocksCSV(AVLTree<Stock>& stockTree, const std::string& filename) {
    ...
    while (std::getline(inFile, line)) {
        ...
        std::string companyName, stockSymbol, priceStr, marketCapStr, volumeStr;

        // Parse fields separated by commas
        if (!std::getline(ss, companyName, ',')) continue;
        if (!std::getline(ss, stockSymbol, ',')) continue;
        if (!std::getline(ss, priceStr, ',')) continue;
        if (!std::getline(ss, marketCapStr, ',')) continue;
        if (!std::getline(ss, volumeStr, ',')) continue;

        try {
            double stockPrice = std::stod(priceStr);
            // Optionally, parse additional fields if needed:
            // long long marketCap = std::stoll(marketCapStr);
            // long long volume = std::stoll(volumeStr);

            stockTree.insert(Stock(companyName, stockSymbol, stockPrice));
        }
